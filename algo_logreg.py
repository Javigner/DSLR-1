...
class Algo:
	...
	def fit_logreg(self, alpha=1, iter=150, lbda=1):

		def gradient_descent(X, y, theta, alpha=1, iter=150, lbda=0.01):
			m = X.shape[0]
		...
		for h in HOUSES:
			...
			self.theta_norm[h], self.J_history[h] = gradient_descent(X, y, theta)
		return None
	...
	def ft_graph_cverrors(self):
		...
		for h,i in zip(HOUSES, range(4)):
			l1 = ax[i // 2, i % 2].scatter(data_errors['taining_errors'].index, data_errors['taining_errors'][h])
	...
	def ft_find_best(self, alpha, iter):
		data_errors = self.ft_read_cv_pickle()
		mse = data_errors['taining_errors'] + data_errors['testing_errors']
		best_lambda = mse.min(axis=1).idxmin()
		self.fit_logreg(alpha, iter,lbda=best_lambda)
		return None
	...
	def ft_read_cv_pickle(self):
		with open('cv_errors.pkl', 'rb') as f:
